...
class Args(Tap):
    ...
    try:
        ...
        branch: str = '[no-git]'  # Default value when git is not available
    ...
    try:
        ...
        commit_id: str = '[no-git]'  # Default value when git is not available
    
    try:
        commit_msg: str = subprocess.check_output(
            'git log -1 --pretty=%B 2>/dev/null', 
            shell=True, stderr=subprocess.DEVNULL
        ).decode('utf-8').strip()
    except (subprocess.CalledProcessError, FileNotFoundError):
        commit_msg: str = '[no-git]'  # Default value when git is not available
