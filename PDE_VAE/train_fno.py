"""
Two-Stage Inverse Problem Solver using VQVAE + FNO

Architecture:
- Stage 1: U-Net for feature extraction from averaged images (1 channel -> 32 channels)
- Stage 2: Fourier Neural Operator (FNO) for inverse mapping to latent space

The FNO replaces the previous Transformer architecture, providing:
- Better handling of spatial relationships through spectral convolutions
- Efficient processing in Fourier domain
- Strong performance on PDE-related inverse problems
"""
...
# ==================== Stage2: Fourier Neural Operator (FNO) ====================
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        batchsize = x.shape[0]
        # Compute Fourier coefficients up to factor of e^(-constant)
...
class FNOInverseNetwork(nn.Module):
    """
    Fourier Neural Operator for inverse mapping from features to latent space.
    Adapted for 2D spatial data with multiple channels.
    """
    def __init__(self, 
                 channels: int = 32,
                 modes1: int = 8, 
        ...
        """
        Args:
            channels: Number of input/output channels from Stage 1
            modes1, modes2: Number of Fourier modes in each dimension
            width: Width of the FNO hidden layers
            n_layers: Number of Fourier layers
        """
        super(FNOInverseNetwork, self).__init__()
        ...
        # Input projection: map from input channels to FNO width
        self.fc0 = nn.Conv2d(channels, self.width, 1)
        ...
        # Output projection: map from FNO width back to output channels
        self.fc1 = nn.Conv2d(self.width, self.width, 1)
        self.fc2 = nn.Conv2d(self.width, channels, 1)
    ...
    def forward(self, x):
        ...
        for i in range(self.n_layers):
            x1 = self.conv_layers[i](x)  # Spectral convolution
            x2 = self.w_layers[i](x)      # Pointwise convolution
            x = x1 + x2
...
class TwoStageInverseNetwork(nn.Module):
    """Complete two-stage network: U-Net + Fourier Neural Operator"""
    def __init__(self, in_channels=1, latent_channels=32, fno_modes=8, fno_width=128, fno_layers=8):
        super().__init__()
        self.in_channels = in_channels
        self.latent_channels = latent_channels
        
        # Stage 1: U-Net for feature extraction
        self.stage1_unet = SimpleConvStage1(in_channels, latent_channels)
        
        # Stage 2: FNO for inverse mapping (replacing Transformer)
        self.stage2_fno = FNOInverseNetwork(
    ...
    def forward(self, x):
        # Stage 1: Learn features using U-Net
        features = self.stage1_unet(x)

        # Stage 2: Inverse mapping using FNO
        latent_pred = self.stage2_fno(features)
...
class VQVAEImagePredictionPipeline:
    ...
    def __init__(self, vqvae_path, device='cuda', checkpoint_dir='checkpoints_fno_nasty35',
                 fno_modes=8, fno_width=128, fno_layers=8):
        """
        Args:
            vqvae_path: Path to pretrained VQVAE checkpoint
            device: Device to run on ('cuda' or 'cpu')
            checkpoint_dir: Directory to save checkpoints
            fno_modes: Number of Fourier modes for FNO (default: 8)
            fno_width: Hidden dimension width for FNO (default: 128)
            fno_layers: Number of FNO layers (default: 8)
        """
        self.device = device
        ...
        # Initialize two-stage network with FNO
        print(f"Initializing Two-Stage Network with FNO (modes={fno_modes}, width={fno_width}, layers={fno_layers})...")
        self.model = TwoStageInverseNetwork(
            in_channels=1, 
            latent_channels=32,
            fno_modes=fno_modes,
            fno_width=fno_width,
...
def main():
    ...
    checkpoint_dir = 'checkpoints_fno_nasty35'
