...
class VQVAE(nn.Module):
    ...
    def img_to_idxBl_with_residuals(
        self, 
        ...
        """
        Encode image to tokens and accumulated residuals.
        Returns: (token_list, residual_list)
        """
        f = self.quant_conv(self.encoder(inp_img_no_grad))
        return self.quantize.f_to_idxBl_or_fhat_with_residuals(
            f, 
            v_patch_nums=v_patch_nums, 
            used_patch_nums=used_patch_nums
        )


    def idxBl_to_img_with_residuals(
        self, 
        ...
        """
        Reconstruct from tokens and accumulated residuals.
        """
        B = ms_idx_Bl[0].shape[0]
        ms_h_BChw = []
        for idx_Bl in ms_idx_Bl:
        ...
        # Reconstruct using residuals
        fhat_results = self.quantize.embed_to_fhat_with_residuals(
            ms_h_BChw=ms_h_BChw,
        ...
        # Decode to image
        if last_one:
            return self.decoder(self.post_quant_conv(fhat_results)).clamp_(-1, 1)
        else:
            return [
                self.decoder(self.post_quant_conv(f_hat)).clamp_(-1, 1) 
                for f_hat in fhat_results
            ]